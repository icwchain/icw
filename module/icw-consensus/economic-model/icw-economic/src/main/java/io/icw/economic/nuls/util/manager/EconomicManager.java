package io.icw.economic.nuls.util.manager;
...
import io.icw.economic.nuls.model.bo.*;
...
//	    	Log.info("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
    ...
    public static List<CoinTo> getRewardCoin2(List<AgentInfo> agentInfos, RoundInfo roundInfo, ConsensusConfigInfo consensusConfig, long unlockHeight, Map<String, BigInteger> awardAssetMap)throws NulsException{
        ...
        for(AgentInfo agentInfo : agentInfos) {
        	BigInteger selfAllDeposit = agentInfo.getDeposit().add(agentInfo.getTotalDeposit());
        	totalWeight += selfAllDeposit.doubleValue();
        }
        ...
        BigDecimal totalAll = BigDecimal.valueOf(inflationInfo.getAwardUnit() / 2 * 150);
        
        Log.info("本轮次Agent数量{}，本轮奖励总额：{}",agentInfos.size(), totalAll);
        
        for(AgentInfo agentInfo : agentInfos) {
        	awardAssetMap = new HashMap<>(NulsEconomicConstant.VALUE_0F_4);
        	if (BigIntegerUtils.isEqual(agentInfo.getDeposit(), BigInteger.ZERO)) {
        		continue;
//                if(awardAssetMap == null || awardAssetMap.isEmpty()){
//                	continue;
