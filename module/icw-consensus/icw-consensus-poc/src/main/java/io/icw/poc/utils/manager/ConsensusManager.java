...
import io.icw.economic.nuls.model.bo.DepositInfo;
...
import io.icw.poc.model.bo.BlockData;
...
public class ConsensusManager {
    ...
    public void addConsensusTx(Chain chain, BlockHeader bestBlock, List<Transaction> txList, MeetingMember self, MeetingRound round, BlockExtendsData extendsData) throws Exception {
        String stateRoot;
        Transaction coinBaseTransaction = createCoinBaseTx(chain, self, txList, round, 0, extendsData);
     ...
     * @return Transaction
     */
    public Transaction createCoinBaseTx(Chain chain, MeetingMember member, List<Transaction> txList, MeetingRound localRound, long unlockHeight, BlockExtendsData extendsData) throws Exception {
...
//	        				);
	        			if (AddressTool.getStringAddressByBytes(coinTo.getAddress()).equals(AddressTool.getStringAddressByBytes(coin.getAddress()))
	        				...
	        				coinTo.setAmount(coinTo.getAmount().add(coin.getAmount()));
	        				exists = true;
	        				break;
	        			}
	        		}
    ...
    private List<CoinTo> calcReward(Chain chain, List<Transaction> txList, MeetingMember self, MeetingRound localRound, long unlockHeight) throws NulsException {
        ...
        */
        ...
        for (Transaction tx : txList) {
            ...
            if (txType != 300 && txType != TxType.COIN_BASE && txType != TxType.CONTRACT_TRANSFER && txType != TxType.CONTRACT_RETURN_GAS && txType != TxType.CONTRACT_CREATE_AGENT
